...
function Main() {
  ...
  const [larivaarAssist, setLarivaarAssist] = useState({
    state: false,
    lineIndex: 0,
    expand: false,
  });
  ...
  const [containerRef, setContainerRef] =
    useState<React.MutableRefObject<any>>();
  const [showFavourites, setShowFavourites] = useState(false);
  const [logo, setLogo] = useState({
    favourites: "💙",
  });
  const [searchMethod, setSearchMethod] = useState(
    SearchMethods["firstLetter"]
  );
  ...
  const customisations = [
    ...
    <div key="displayMode" className="customisation">
      ...
      <Switch
        ...
        checked={mode === "light"}
        onClick={(e) => {
          const modeVal = (e.target as HTMLInputElement).checked
            ? "light"
            : "dark";
          setMode(modeVal);
    ...
    <div key={"larivaar"} className="customisation">
      <Checkbox
        checked={isLarivaar}
        className="checkbox"
        onChange={(e) => setIsLarivaar(e.target.checked)}
        label="Larivaar"
      />
      {isLarivaar && (
        <Checkbox
          style={{ marginLeft: "1rem" }}
          checked={larivaarAssist.state}
          className="checkbox"
          onChange={(e) =>
            setLarivaarAssist({ ...larivaarAssist, state: e.target.checked })
          }
    ...
    <div key={"vishraams"} className="customisation">
      <Checkbox
        checked={vishraams}
        className="checkbox"
        onChange={(e) => setVishraams(e.target.checked)}
        label="Vishraams"
      />
    </div>,
    ...
    <div
      style={{ display: showCustomisations ? "flex" : "none" }}
      key={"enArth"}
    ...
    >
      <Checkbox
        ...
        className="checkbox"
        onChange={(e) => setShowEnglishMeaning(e.target.checked)}
    ...
    >
      <Checkbox
        checked={showPunjabiMeaning}
        className="checkbox"
        onChange={(e) => setShowPunjabiMeaning(e.target.checked)}
        label="ਗੁਰਮੁਖੀ ਅਰਥ"
      />
    </div>,
    <div
    ...
    >
      <Checkbox
        checked={scrolling.status}
        className="checkbox"
        onChange={(e) =>
          setScrolling({ ...scrolling, status: e.target.checked })
        }
        label="Auto Scroll"
      />
    </div>,
    ...
    <div
      style={{ display: scrolling.status ? "flex" : "none" }}
      key={"scrollSpeed"}
      className="customisation"
    >
      <ButtonGroup size="sm" aria-label="Scroll Speed">
        <Button
          onClick={() =>
            setScrolling({ ...scrolling, speed: scrolling.speed + 1 })
          }
        >
          +
        </Button>
  ...
  if (mode === "dark") {
    root.style.setProperty("--mainFontColor", "#fafafa");
    root.style.setProperty("--mainBackgroundColor", "#181818");
    ...
    root.style.setProperty("--headerBackgroundColor", "#181818");
  ...
  const handleFavourites = () => {
    if (location.pathname === "/bani") {
      ...
      const shabadAlreadyPresent = currFavourites.filter(
        (curr) => curr.shabadId === currentShabad.shabadId
      ).length;
      if (!shabadAlreadyPresent) {
        ...
        const newFavourites = [...currFavourites].filter(
          (ele) => currentShabad.shabadId !== ele.shabadId
        );
        saveToLS("favourites", newFavourites);
      }
      return;
  ...
  if (!getFromLS("favouritesReversed")) {
    let fav = getFromLS("favourites"),
      hist = getFromLS("history");
  ...
  return (
    ...
    >
      <div
        className={
          expandCustomisations ? "expandCustomisations" : "customisations"
        }
      >
        <div
          className={
            expandCustomisations ? "buttonGroupNoMarginTop" : "buttonGroup"
          }
        >
          <svg
            onClick={() => setExpandCustomisations(!expandCustomisations)}
          ...
          {expandCustomisations && (
            <>
              <Button
                style={{ display: installationPrompt ? "block" : "none" }}
                onClick={() => {
