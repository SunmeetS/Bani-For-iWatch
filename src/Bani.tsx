...
import { Info, getFirstLetters, removeMatras, utils } from "./utils";
...
const Bani = ({ baniId, shabadId }) => {
  ...
  const scrollToFoundShabad = () => {
    if (foundShabadIndex?.current) {
      const shabadChildren: (HTMLElement | undefined)[] =
  ...
  const handleSearch = (bani = null) => {
    ...
    for (let i = 0; i < baniData.details.length; i++) {
      ...
      const tukFirstLetters: string =
        getFirstLetters(tukWithoutMatras).join("");
      const firstLettersSearch: string = removeMatras(search)
        .split(" ")
        .join("");
      ...
      if (searchMethod === SearchMethods.tuk) {
        ...
        if (
          tukFirstLetters?.includes(firstLettersSearch) ||
          tuk === shabadTuk
        ) {
          if (shabadId) {
  ...
  return (
    <div className="App">
      <div className="Bani" ref={containerRef}>
        ...
        {isWrap &&
          wrapTuk.map((ele, index) => {
            return (
              <span
                style={{ fontSize: fontSize }}
                className={
                  larivaarAssist.state && index % 2 ? "larivaarAssist" : ""
                }
              >
                {ele + (!isLarivaar ? " " : "")}
              </span>
        ...
        {!isWrap &&
          baniData?.details?.map((verse, idx) => {
            let { tuk, englishTuk, englishMeaning, punjabiMeaning, vishraam } =
              verse ?? {};
            const vishraamIndexMajor = vishraam?.[0]?.p,
              vishraamIndexMinor = vishraam?.[1]?.p;
            ...
            return (
              <div className={className2}>
                ...
                >
                  {isEnglish
                    ? englishTuk
                    : tuk?.map((ele: string, index: number) => {
                        return (
                          <span
                            className={`${
                              larivaarAssist.state &&
                              index % 2 &&
                              idx != 0 &&
                              "larivaarAssist"
                            } ${
                              vishraamIndexMajor === index &&
                              vishraams &&
                              "vishraamMajor"
                            } ${
                              vishraamIndexMinor === index &&
                              vishraams &&
                              "vishraamMinor"
                            }`}
                            style={{
                              marginRight:
                                larivaarAssist.expand &&
                                larivaarAssist.lineIndex === idx &&
                                "10px",
                ...
                {(englishMeaning || punjabiMeaning) && (
                  <div className="meaningsGroup">
                    {showEnglishMeaning && (
                      <p
                        style={{ fontSize: fontSize / 2 }}
                        className="englishMeanings"
                      >
                        {englishMeaning}
                      </p>
                    )}
        ...
        {shabadId && (
          <div className="shabadNavigation">
            <Button
              onClick={() => {
